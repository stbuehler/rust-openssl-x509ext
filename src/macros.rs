...
macro_rules! private_key_from_pem {
    ($(#[$m:meta])* $n:ident, $(#[$m2:meta])* $n2:ident, $(#[$m3:meta])* $n3:ident, $t:ty, $f:path) => {
        ...
        $(#[$m2])*
        pub fn $n2(pem: &[u8], passphrase: &[u8]) -> Result<$t, openssl::error::ErrorStack> {
            unsafe {
                ffi::init();
        ...
        $(#[$m3])*
        pub fn $n3<F>(pem: &[u8], callback: F) -> Result<$t, openssl::error::ErrorStack>
            where F: FnOnce(&mut [u8]) -> Result<usize, openssl::error::ErrorStack>
        {
            unsafe {
                ffi::init();
...
macro_rules! private_key_to_pem {
    ($(#[$m:meta])* $n:ident, $(#[$m2:meta])* $n2:ident, $f:path) => {
        $(#[$m])*
        pub fn $n(&self) -> Result<Vec<u8>, openssl::error::ErrorStack> {
            unsafe {
...
macro_rules! from_der {
    ($(#[$m:meta])* $n:ident, $t:ty, $f:path) => {
        $(#[$m])*
        pub fn $n(der: &[u8]) -> Result<$t, openssl::error::ErrorStack> {
            unsafe {
                ::ffi::init();
...
macro_rules! from_pem {
    ($(#[$m:meta])* $n:ident, $t:ty, $f:path) => {
        $(#[$m])*
        pub fn $n(pem: &[u8]) -> Result<$t, openssl::error::ErrorStack> {
            unsafe {
                openssl::init();
                let bio = crate::bio::MemBioSlice::new(pem)?;
